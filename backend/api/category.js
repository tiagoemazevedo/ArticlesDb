module.exports = app => {
    ...
    const remove = async (req,res) => {
        //not a soft delete
        try {
            existsOrError(req.params.id, 'Category Id required')

            const subcategory = await app.db('categories').where({ parentId: req.params.id })
            notExistsOrError(subcategory, 'Category has subcategories, delete them first')

            const articles = await app.db('articles').where({ categoryId: req.params.id })
            notExistsOrError(articles, 'Category has articles, delete them first')
    ...
    //create tree to display in the Navbar with categories
